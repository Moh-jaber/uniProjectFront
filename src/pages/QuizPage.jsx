...
const QuizPage = () => {
  ...
  const handleAnswer = (value) => {
    ...
    const newScores = { ...scores, [question.dimension]: scores[question.dimension] + points };
    setScores(newScores);
  ...
  const finishQuiz = (finalScores) => {
    ...
    history.unshift({ type, date: new Date().toISOString() });
        ...
        <div className="container" style={{ position: 'relative', zIndex: 1, maxWidth: '900px' }}>
          <div className="animate-fade-in" style={{ 
            ...
            <div style={{ marginBottom: '3rem' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem', color: '#666', fontSize: '0.9rem', fontWeight: '500' }}>
                <span>Question {currentQuestion + 1} of {questions.length}</span>
                <span>{Math.round(progress)}% Completed</span>
              </div>
              <div style={{ width: '100%', background: 'rgba(255,255,255,0.5)', height: '10px', borderRadius: '5px', overflow: 'hidden' }}>
                <div style={{ width: `${progress}%`, background: 'var(--primary-gradient)', height: '100%', borderRadius: '5px', transition: 'width 0.5s ease' }}></div>
              </div>
            </div>
            
            <h2 style={{ margin: '0 0 3rem 0', fontSize: '2.5rem', lineHeight: '1.3' }}>{questions[currentQuestion].text}</h2>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
              ...
              >
                <span style={{ fontSize: '1.5rem' }}>👍</span> Strongly Agree
              </button>
              ...
              >
                <span style={{ fontSize: '1.5rem' }}>👎</span> Strongly Disagree
              </button>
            </div>
          </div>
        </div>
